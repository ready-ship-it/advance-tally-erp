...
from data.indian_states import INDIAN_STATES
...
def edit_category(cid):
    ...
    if request.method == "POST":
        ...
        c.default_hsn = request.form.get("default_hsn", "").strip()
        c.default_gst_rate = float(request.form.get("default_gst_rate", 18) or 18)
        db.session.commit()
...
def parties():
    ...
    return render_template("parties.html", parties=items, q=q)
...
def new_party():
    ...
    from data.indian_states import INDIAN_STATES
    return render_template("party_form.html", party=None, states=INDIAN_STATES)
...
@bp.route("/retail-customers/new", methods=["GET", "POST"])
@admin_required
def new_retail_customer():
    ...
    if request.method == "POST":
        ...
        db.session.add(p)
        db.session.commit()
        flash("Retail customer added", "success")
        return redirect(url_for("inventory.retail_customers"))
    from data.indian_states import INDIAN_STATES
    return render_template("retail_customer_form.html", customer=None, states=INDIAN_STATES)


@bp.route("/retail-customers/<int:pid>/edit", methods=["GET", "POST"])
@admin_required
def edit_retail_customer(pid):
    """Edit a retail customer."""
    p = Party.query.get_or_404(pid)
    ...
    if request.method == "POST":
        ...
        p.email = request.form.get("email", "")
        db.session.commit()
        flash("Retail customer updated", "success")
        return redirect(url_for("inventory.retail_customers"))
    from data.indian_states import INDIAN_STATES
...
def delete_retail_customer(pid):
    ...
    db.session.delete(p)
    db.session.commit()
    flash("Retail customer deleted", "success")
    return redirect(url_for("inventory.retail_customers"))
